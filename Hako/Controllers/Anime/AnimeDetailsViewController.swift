...
class AnimeDetailsViewController: ObservableObject {
    ...
    func loadDetails() async {
        ...
        do {
            ...
            withAnimation {
                ...
                relatedAnime = prequels + sequels + others.sorted(by: { ($0.relation ?? "") < ($1.relation ?? "") })
            }
    ...
    func loadRelatedManga() async {
        ...
        relatedLoadingState = .loading
        do {
            relatedManga = try await networker.getAnimeRelations(id: id)
    ...
    func loadReviews() async {
        reviewsLoadingState = .loading
        do {
            let reviews = try await networker.getAnimeReviewsList(id: id, page: 1, sentiment: .all)

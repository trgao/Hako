                            ...
                            if controller.isLoadingError && controller.animeItems.isEmpty {
                                ...
                                LazyVGrid(columns: columns) {
                                    ForEach(controller.animeItems, id: \.node.id) { item in
                                        AnimeGridItem(id: item.node.id, title: item.node.title, imageUrl: item.node.mainPicture?.large, subtitle: rankToString(item.ranking?.rank))
                                            .task {
                            ...
                            if controller.isLoadingError && controller.mangaItems.isEmpty {
                                ...
                                LazyVGrid(columns: columns) {
                                    ForEach(controller.mangaItems, id: \.node.id) { item in
                                        MangaGridItem(id: item.node.id, title: item.node.title, imageUrl: item.node.mainPicture?.large, subtitle: rankToString(item.ranking?.rank))
                                            .task {
                                                await controller.loadMoreIfNeeded(currentItem: item)
                                            }
                                    }

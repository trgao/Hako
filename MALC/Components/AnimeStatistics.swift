...
struct AnimeStatistics: View {
    @StateObject private var controller: AnimeStatisticsController
    
    init(id: Int) {
        self._controller = StateObject(wrappedValue: AnimeStatisticsController(id: id))
    }
    ...
    var body: some View {
        if let statistics = controller.statistics {
            Section {
                ...
                ListRow(title: "Plan to watch", content: String(statistics.planToWatch), icon: "plus.circle.dashed", color: .cyan)
            } header: {
                Text("Statistics")
                    .textCase(nil)
